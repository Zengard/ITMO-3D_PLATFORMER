...
public class PlayerV3Movement : MonoBehaviour
{
    ...
    protected bool CheckGround()
    {
        ...
        if (Physics.Raycast(transform.position - new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down), detectCollider + length, playerMask)
            || Physics.Raycast(transform.position + new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down), detectCollider + length, playerMask))
        {
            Debug.DrawRay(transform.position - new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down) * (GetComponent<BoxCollider>().bounds.extents.y + length), Color.green);
            Debug.DrawRay(transform.position + new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down) * (GetComponent<BoxCollider>().bounds.extents.y + length), Color.green);
            return true;
        }
        else
