...
public class PlayerV2Movement : MonoBehaviour
{
    public float invincibility = 3;
    ...
    //shoot variable
    [SerializeField] GameObject[] bulletsPool = new GameObject[4];
    ...
    private void Awake()
    {
        ...
        detectCollider = GetComponent<BoxCollider>().bounds.extents.y;

        isWalkingHash = Animator.StringToHash("isWalking");
        isJumpingHash = Animator.StringToHash("isJumping");
        isShootHash = Animator.StringToHash("isShooting");
        ...
        playerInputMaster.Player.Shoot.started += Shoot;
        playerInputMaster.Player.Shoot.canceled += Shoot;

        playerInputMaster.Player.Switch.started += Switch;
        playerInputMaster.Player.Switch.canceled += Switch;
        SetUpJump();
    ...
    private void HandleSwitch()
    {
        if (isSwitchPressed)
        {
            switchCharacter.version--; 
            switchCharacter.Switch(this.gameObject);

             if (switchCharacter.version < 1)
            {
                switchCharacter.version = 1;
            }
        }
    }
    ...
    private void HandleShoot()
    {
        if (isShootPressed)
        {
            playerAnimator.SetBool(isShootHash, true);

            if (coolDownTimer > attackCoolDown)
            {
                foreach (var shoot in bulletsPool)
                 {
                    if (shoot.gameObject.activeSelf == false)
                    {                   
        ...
        {
            playerAnimator.SetBool(isShootHash, false);
        }
    }
    ...
    private void HandleRotation()
    {
        ...
        if (rotateR == true)
        {
            ...
            bulletPosition.transform.localScale = new Vector3(1, 0, 0);
    ...
    protected bool CheckGround()
    {
        ...
        if (Physics.Raycast(transform.position - new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down), detectCollider + 0.1f, playerMask)
            || Physics.Raycast(transform.position + new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down), detectCollider + 0.1f, playerMask))
        {
            Debug.DrawRay(transform.position - new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down) * (GetComponent<BoxCollider>().bounds.extents.y + 0.1f), Color.green);
            Debug.DrawRay(transform.position + new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down) * (GetComponent<BoxCollider>().bounds.extents.y + 0.1f), Color.green);
            return true;
        }
        else
        {
            Debug.DrawRay(transform.position - new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down) * (GetComponent<BoxCollider>().bounds.extents.y + 0.1f), Color.red);
            Debug.DrawRay(transform.position + new Vector3(GetComponent<BoxCollider>().bounds.extents.x * TEST, 0, 0), transform.TransformDirection(Vector2.down) * (GetComponent<BoxCollider>().bounds.extents.y + 0.1f), Color.red);
            return false;
        }
    }
